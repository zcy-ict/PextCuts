...
int inline CompareQuery(const box& a, const PSPacket& q, int level, const std::vector<int>& fieldOrder) {
...
int RBExactQuery( rb_red_blk_tree*  tree, const PSPacket& q,int level, const std::vector<int>& fieldOrder) {
...
int RBExactQueryIterative(rb_red_blk_tree*  tree, const PSPacket& q, const std::vector<int>& fieldOrder) {
...
int RBExactQueryIterativeAccess(rb_red_blk_tree*  tree, const PSPacket& q, const std::vector<int>& fieldOrder, ProgramState *program_state) {
...
int RBExactQueryPriority(rb_red_blk_tree*  tree, const PSPacket& q, int level, const std::vector<int>& fieldOrder, int priority_so_far) {
...
int RBExactQueryPriorityAccess(rb_red_blk_tree*  tree, const PSPacket& q, int level, const std::vector<int>& fieldOrder, int priority_so_far) {
...
/*              
/**/
/*    OUTPUT:  a vector of PSRule */
/**/
/*    Modifies Input: none */
/***********************************************************************/

void  RBSerializeIntoRulesRecursion(rb_red_blk_tree * treenode, rb_red_blk_node * node, int level, const std::vector<int>& fieldOrder, std::vector<box>& box_so_far, std::vector<PSRule>& rules_so_far) {
	if (level == fieldOrder.size() ) {
		for (int n : treenode->priority_list) {
			PSRule r(fieldOrder.size());
...
std::vector<PSRule> RBSerializeIntoRules(rb_red_blk_tree* tree, const std::vector<int>& fieldOrder) {
	std::vector<box> boxes_so_far;
	std::vector<PSRule> rules_so_far;
...
int  CalculateMemoryConsumptionRecursion(rb_red_blk_tree * treenode, rb_red_blk_node * node, int level, const std::vector<int>& fieldOrder) {
	if (level == fieldOrder.size()) {
		...
		return (num_rules_in_this_leaf + also_max_priority)*sizeofint + sizeof(rb_red_blk_tree);
	...
	if (treenode->count == 1) {
		...
		return   size_of_remaining_intervals + (num_rules_in_this_leaf + also_max_priority)*sizeofint + sizeof(rb_red_blk_tree);
	}

	if (treenode->nil == node) return  sizeof(rb_red_blk_tree);
	
	int memory_usage =  sizeof(rb_red_blk_tree); 
	auto tree = node->rb_tree_next_level;
