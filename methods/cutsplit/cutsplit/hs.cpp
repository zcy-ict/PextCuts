...
void FreeHSTree(hs_node_t* node, bool free_self) {
	if (node == NULL)
		return;
	if (node->child[0]) {
		FreeHSTree(node->child[0], true);
		FreeHSTree(node->child[1], true);
	} else {
		free(node->ruleset->ruleList);
		free(node->ruleset);
	}
...
uint64_t HSMemorySize(hs_node_t* node) {
	if (node == NULL)
		return 0;
	uint64_t memory_size = sizeof(hs_node_t);
	if (node->child[0]) {
		memory_size += HSMemorySize(node->child[0]);
		memory_size += HSMemorySize(node->child[1]);
	} else {
		memory_size += node->ruleset->num * sizeof(rule_t);
		memory_size += sizeof(rule_set_t);
	}
	return memory_size;
...
int HSHeight(hs_node_t* node) {
	if (node == NULL)
		return 0;
	if (node->child[0]) {
		int height = max(HSHeight(node->child[0]), HSHeight(node->child[1]));
		return height + 1;
	} else {
		return 1;
	}
...
int HSRealHeight(hs_node_t* node) {
	if (node == NULL)
		return 0;
	if (node->child[0]) {
		int height = max(HSRealHeight(node->child[0]), HSRealHeight(node->child[1]));
		return height + 1;
	} else {
		return 1 + node->ruleset->num;
	}
}
...
int HSCalNum(hs_node_t* node, ProgramState *program_state) {
	...
	if (node->child[0]) {
		...
		HSCalNum(node->child[0], program_state);
		HSCalNum(node->child[1], program_state);
	} else {
		program_state->tree_rules_num += node->ruleset->num;
		program_state->tree_real_rules_num += node->ruleset->num;
	}
	return 0;
}